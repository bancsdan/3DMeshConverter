...
void StlWriter::writeTriangles(std::ofstream& out_file,
  ...
  if (needs_byte_swap) {
      for (const auto& triangle : mesh.m_triangles) {
          ...
          Eigen::Vector4f cross = (triangle.m_b.m_pos - triangle.m_a.m_pos).cross3(triangle.m_c.m_pos - triangle.m_a.m_pos).cast<float>();
          cross.normalize();
          ...
          out_file.write((const char*)triangle_coords.data(), triangle_coords.size()*sizeof(float));
    ...
    for (const auto& triangle : mesh.m_triangles) {
          ...
          Eigen::Vector4f cross = (b_pos - a_pos).cross3(c_pos - a_pos).cast<float>();
          cross.normalize();
